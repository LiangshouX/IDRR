"""
    实现LDSGM模型的各个部分
        LDSGM模型总体上由三个部分组成：用于学习数据全局表示和特定于级别的上下文Label Attentive Encoder、
            用于预测生成更高级别标签依赖关系的Label Sequence Decoder和自底向上的标签依赖训练模型Mutual Learning

        其中Encoder部分堆叠的Transformer应用了预训练的RoBERTa模型
"""
...
class Label2Context(nn.Module):
    """Encoder中的label attention
    """
