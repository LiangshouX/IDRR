...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, y1, y2, argmask) in enumerate(train_iter):
            ...
            outputs_top, outputs_sec, outputs_conn, outputs_top_reverse, outputs_sec_reverse, outputs_conn_reverse = \
                model(trains, argmask)
            ...
            if config.show_time:
                if total_batch % 100 == 0:
                    ...
                    if config.need_clc_loss:
                        ...
                        y_predit_conn = outputs_conn.data.cpu()
                    ...
                    loss_dev, acc_top, f1_top, acc_sec, f1_sec, acc_conn, f1_conn = evaluate(config, model, test_iter)
                    ...
                    msg = 'top-down:TOP: Iter: {0:>6},  Train Loss: {1:>5.2},  Train Acc: {2:>6.2%},' + \
                          'Test Loss: {3:>5.2},  Test Acc: {4:>6.2%}, Test F1: {5:>6.2%} Time: {6} {7}'
                    ...
                    #       'Test Loss: {3:>5.2},  Test Acc: {4:>6.2%}, Test F1: {5:>6.2%} Time: {6} {7}'
                    # lgg.info(msg.format(total_batch, loss.item(), train_acc_sec, loss_dev, acc_sec, f1_sec, time_dif,
                    #                     improve))
    ...
    dev_msg = 'Test_best_acc_top: {0:>6.2%},  Test_best_f1_top: {1:>6.2%}, \n' + \
              'Test_best_acc_sec: {2:>6.2%},  Test_best_f1_sec: {3:>6.2%}, \n' + \
              'Test_best_acc_conn: {4:>6.2%},  Test_best_f1_conn: {5:>6.2%}'
...
def test(config, model, test_iter, reverse=False, ensemble=False):
    ...
    acc_sec, f1_sec, report_sec, confusion_sec, acc_conn, f1_conn = evaluate(config, model, test_iter, test=True,
...
def evaluate(config, model, data_iter, test=False, reverse=False, ensemble=False):
    ...
    if test:
        report_top = metrics.classification_report(gold_sense_top, predict_sense_top, target_names=config.i2top,
                                                   digits=4)
        confusion_top = metrics.confusion_matrix(gold_sense_top, predict_sense_top)

        report_sec = metrics.classification_report(gold_sense_sec, predict_sense_sec, target_names=config.i2sec,
                                                   digits=4)
        confusion_sec = metrics.confusion_matrix(gold_sense_sec, predict_sense_sec)

        return loss_total / len(
            data_iter), acc_top, f1_top, report_top, confusion_top, acc_sec, f1_sec, report_sec, confusion_sec, acc_conn, f1_conn
