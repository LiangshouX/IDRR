...
class Config(object):
    ...
    Args:
        dataset(str): 数据集的根目录
        cuda(int): cuda编号，默认为0
        finetune(bool): 是否微调
        bertmodel(str): bert模型
        lambda4(float): KL loss的lambda值
    """
    def __init__(self, dataset, cuda=0, finetune=True, bertmodel='roberta', lambda4=1.0):
        ...
        self.log = dataset + '/log/' + self.model_name + '_' + str(t).replace(':', '') + '.log'
        print(self.log)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Label Dependence-aware Sequence Generation Model for Multi-level Implicit Discourse Relation '
                    'Recognition')
